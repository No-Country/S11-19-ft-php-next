...
import GhBrand from "../assets/gh-brand.jpg"
...
const Header = () => {
	...
	return (
		<header >
			<div className="flex flex-row justify-between items-center h-[4.5em] px-3 bg-primary">
			{isLogged ? (
				<>
					<button id="hamburgerMenu" className="p-3 border-solid border lg:hidden border-slate-100 rounded-lg" onClick={openTheMenu}>
            <Image src={Hamburger} alt="humburger button icon" />
					{/* <svg xmlns="http://www.w3.org/2000/svg" width="20" height="auto" viewBox="0 0 20 20" fill="none">
						<path d="M0 3H20V5H0V3ZM0 9H20V11H0V9ZM0 15H20V17H0V15Z" fill="white"/>
					</svg> */}
					</button>
					{/* <Brand/> */}
					<Image src={GhBrand} className=" w-[10em]  md:w-[12.5em] pl-4 sm:pl-5 md:pl-3 lg:pl-1" alt="bell icon" />
					...
					<nav ref={menuRef}  className={`fixed top-0 left-0 lg:static  lg:h-full lg:text-base lg:bg-transparent lg:w-1/2 lg:text-slate-100 bg-slate-100 transition-transform duration-500 text-primary text-xl text-center h-screen  z-50 w-52 ${isOpen ? 'translate-x-0': '-translate-x-full lg:-translate-x-0'}`}>
						<ul className="lg:flex justify-between items-center lg:h-full">
							...
							<li className="my-10 lg:my-0"><Link href={'/calendar'}>Calendario</Link></li>
							<li className="my-10 lg:my-0"><Link href={'/reminder'}>Recordatorio</Link></li>
					...
					<div className="flex justify-between text-slate-100 space-x-5 items-center">
						<Link href={''}>
							{/* <svg xmlns="http://www.w3.org/2000/svg" className="hidden lg:block" width="24" height="24" viewBox="0 0 24 24" fill="none">
								<path d="M15 17H20L18.595 15.595C18.4063 15.4063 18.2567 15.1822 18.1546 14.9357C18.0525 14.6891 18 14.4249 18 14.158V11C18.0002 9.75894 17.6156 8.54834 16.8992 7.53489C16.1829 6.52144 15.17 5.75496 14 5.341V5C14 4.46957 13.7893 3.96086 13.4142 3.58579C13.0391 3.21071 12.5304 3 12 3C11.4696 3 10.9609 3.21071 10.5858 3.58579C10.2107 3.96086 10 4.46957 10 5V5.341C7.67 6.165 6 8.388 6 11V14.159C6 14.697 5.786 15.214 5.405 15.595L4 17H9M15 17H9M15 17V18C15 18.7956 14.6839 19.5587 14.1213 20.1213C13.5587 20.6839 12.7956 21 12 21C11.2044 21 10.4413 20.6839 9.87868 20.1213C9.31607 19.5587 9 18.7956 9 18V17" stroke="#FFF4E4" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/>
							</svg> */}
							<Image src={Bell} width={24} height={24} className="hidden lg:block" alt="bell icon" />
						</Link>
						<p className="hidden lg:block">{userState?.name} {userState?.lastname}</p>
						<Link href={'/profile'}>
				...
				<>
					{/* <Brand/> */}
					<Image src={GhBrand} width={200} alt="bell icon" />
